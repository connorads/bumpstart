...
# bump_os — echo mac|win|linux. An explicit $BUMP_OS override wins (the test seam,
...
bump_os() {
  if [ -n "${BUMP_OS:-}" ]; then printf '%s' "$BUMP_OS"; return 0; fi
  case "$(uname -s)" in
    Darwin)                          printf 'mac' ;;
...
# bump_wsl — echo 2, 1 or nothing: which WSL version this is, if any. Reads the
# kernel release string ($BUMP_OSRELEASE_FILE is the test seam) rather than
...
bump_wsl() {
  _vw_file="${BUMP_OSRELEASE_FILE:-/proc/sys/kernel/osrelease}"
  [ -r "$_vw_file" ] || return 0
...
# bump_os_key — echo MAC|WIN|LINUX, the meta cell suffix for the current OS.
bump_os_key() {
  case "$(bump_os)" in
    mac)   printf 'MAC' ;;
    win)   printf 'WIN' ;;
...
#
# Lives here, not in run.sh, because it takes a block DIR and needs only bump_os:
# the resolve drivers source meta + os + resolve alone, and a predicate in run.sh
# would break them. bash-3.2-clean.
block_has_content() {
  [ -f "$1/content.$(bump_os).md" ] && return 0
