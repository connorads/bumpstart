...
Describe 'Get-BumpPathVariants' {
  It 'yields slash + drive-case variants for a Windows path' {
    $v = Get-BumpPathVariants 'C:\Users\me\git\first-project'
  ...
  It 'is deduped and stable for a slash-free path' {
    $v = Get-BumpPathVariants '/home/me/first-project'
    ($v | Select-Object -Unique).Count | Should -Be $v.Count
    $v | Should -Contain '/home/me/first-project'
  }
}

Describe 'Expand-BumpHome' {
  It 'expands a leading $HOME to the home dir' {
    (Expand-BumpHome '$HOME/.claude/CLAUDE.md') | Should -Be (Join-Path $HOME '.claude/CLAUDE.md')
  }
  It 'expands a leading ~' {
    (Expand-BumpHome '~/.codex/AGENTS.md') | Should -Be (Join-Path $HOME '.codex/AGENTS.md')
  }
  It 'leaves an absolute path untouched' {
    (Expand-BumpHome '/etc/thing') | Should -Be '/etc/thing'
  }
}

Describe 'Test-BumpCopyOwned' {
  It 'treats a missing target as ownable' {
    Test-BumpCopyOwned (Join-Path $TestDrive 'nope.md') | Should -BeTrue
  }
...
Describe 'The wizard fold over axes' {
  ...
  It 'puts presets first in a group, then ids alphabetically' {
    ...
    (Get-BumpAxisMembers $script:fix 'agent') -join ' ' | Should -Be 'claude codex claude-cli codex-cli'
    (Get-BumpAxisMembers $script:fix 'recipe') -join ' ' | Should -Be 'starter'
  }

  It 'never offers a block that declares no AXIS' {
    # mise arrives via node; nobody picks a version manager directly
    foreach ($ax in (Get-BumpAxes $script:fix)) {
      Get-BumpAxisMembers $script:fix $ax.Id | Should -Not -Contain 'mise'
    }
  }

  It 'shows what a row pulls in, so wholes and parts read as nested' {
    Get-BumpAxisDetail $script:fix 'starter' | Should -Be ' (pulls in: web beginner)'
    Get-BumpAxisDetail $script:fix 'concise' | Should -Be ''
  }
}
